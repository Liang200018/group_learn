...
# 控制爬取整个讨论的流程和逻辑
def get_all_discussion(url,name):

    url=url+"#/learn/forumindex"
    # 输入要爬取的课程的链接和课程名，注意需要在有讨论页下的链接
    #url = 'https://www.icourse163.org/learn/icourse-1001555013?tid=1001628022#/learn/forumindex'
    # url = 'https://www.icourse163.org/learn/WHU-1207492805?tid=1207844202#/learn/forumindex'
    school_id, course_id = re.findall(r"/learn/([A-Z]+)-(\d+)",url)[0]  # 输入你要爬取的课程名
    course_name=school_id + course_id
    print(course_name)
    
    # 获取学校课程编号以及termId
    schoolCourse = re.findall('[learn||icourse]/(.*?)\?tid=',url)[0]   # schoolCourse = PKU-1207230807
    ...
    fp = open(name+"\\"+'%s_discussion.csv' % (course_name), 'w', newline='', encoding='utf-8')
